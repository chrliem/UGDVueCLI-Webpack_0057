<template>
 <v-main class="list">
   ...
   <v-card>
     ...
     <v-data-table :headers="headers" :items="filterByPriority"  :search="search">
       ...
       <template v-slot:[`item.actions`]="{ item }">
         ...
         <v-checkbox small class="mr-2" @click="selectToDelete(item)"> 
        </v-checkbox>
       </template>
     </v-data-table>
   </v-card>
   ...
   <v-dialog v-model="dialogDelete" max-width="400px">
      <v-card>
        ...
        <v-card v-if="selectedItem.length">
          <v-card-title>
              <h4>Item to Delete:</h4>
          </v-card-title>
          <v-list-item v-for="(item, i) in selectedItem" :key="i">
            <ul>
              <li>{{item.task}}</li> 
            </ul>
           </v-list-item>
            <v-btn @click="deleteSelectedItem">
                DELETE 
            </v-btn>
        </v-card>
...
export default {
 ...
 methods: {
   ...
   deleteSelectedItem(){
      var count =0;
      for(count = 0;count<this.selectedItem.length; count++){
                const index=this.todos.indexOf(this.selectedItem[count]);
                this.todos.splice(index, 1);
            }
      this.selectToDelete(this.todos);
      this.selectedItem=[];
 ...
 computed: {
  filterByPriority(){
        var selected
        let select=this.filters;
      if(select=="All Priority"){
          return this.todos;
      }else{
            selected=this.todos.filter(function(item){
            return item.priority==select;
          })
        return selected;
      }
    },
